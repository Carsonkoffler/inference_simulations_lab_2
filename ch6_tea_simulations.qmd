...
```{r}
bycondition <- tea_data %>%
  group_by(condition) %>%
  summarize(m.rating = mean(rating),
             n.participants = length(rating))

 
bycondition_highn <- tea_data_highn %>%
  group_by(condition) %>%
  summarize(m.rating_highn = mean(rating),
            n.participants_highn = length(rating))
```
...
-   A two-sample t-test revealed a significant difference between milk-first and tea-first tea, t(`r out_high_n$parameter` = `r out_high_n$statistic`, p = `r out_high_n$p.vlaue` \
...
tea_data_summary <- samps |>
  group_by(sim, condition) |> #group together conditions by simulation # 
  summarise(mean_rating = mean(rating)) |> #column for avg rating for each condition by sim
  group_by(sim) |> #group together the simulations
  summarise(delta = mean_rating[condition == "milk first"] - 
              mean_rating[condition == "tea first"]) #find the average difference btwn conditions

#creates df consisting of 2 columns: sim number and avg rating of milk first - tea first 
```
...
```{r}
ggplot(data = tea_data, aes(x=rating)) +
...
```{r}
ggplot(data = tea_data_highn, aes(x=rating)) +
       geom_histogram(alpha = 0.8, bins = 15)
```
...
```{r}
ggplot(data = nullmodel_summary, aes(x=delta)) +
       geom_histogram(alpha = 0.8, bins = 15)
```
...
Sim is the number of simulations (1000).
...
It looks like a normal curve with the mean slightly to the left of 0.0.
